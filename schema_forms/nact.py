...
class NeoAdjuvantChemoDrugForm (SectionForm):
    fld_drug_nact = SelectField("Drug used for therapy", choices=ChemoDict.drugs_choice)
    ...
    fld_cyc_freq_week_nact = IntegerField('Cycle Frequency (frequency per week, so three weekly is 3 and weekly is 1)',
                                          default=3)
    fld_number_cycle_nact = IntegerField('Number of cycles actually given', default=0)
    submit_button = SubmitField('Submit Form')

class NeoAdjuvantChemoToxicityForm (SectionForm):
    fld_tox_nact = SelectField("Were there any toxic effects?", choices=CommonDict.yes_no_choice)
    fld_tox_type_nact = SelectField("Toxic effects", choices=ChemoDict.toxic_side_effects_choice)
    fld_tox_type_other_nact = StringField("Other")
    fld_tox_grade_nact = SelectField("Grade of toxic effects", choices=ChemoDict.toxic_side_effects_grade_choice)
    fld_tox_grade_other_nact = StringField("Other")
    fld_treatment_nact = StringField("Treatment given for toxicity", default=tbd)
    ...
    fld_cyc_tox_nact = IntegerField("Cycle at which toxicity occured")
    submit_button = SubmitField('Submit Form')